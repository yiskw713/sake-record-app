...
describe('getSuggestions', () => {
  ...
  test('酒蔵フィルタで絞り込まれる', () => {
    const results = getSuggestions('久保田', records, '朝日酒造');
    expect(results.length).toBeGreaterThan(0);
    expect(results.every(r => r.brewery === '朝日酒造')).toBe(true);
  });

  test('酒蔵フィルタ時、他の酒蔵の銘柄は含まれない', () => {
    const results = getSuggestions('獺祭', records, '朝日酒造');
    expect(results.every(r => r.brewery === '朝日酒造')).toBe(true);
  });

  test('酒蔵フィルタが空の場合は全銘柄から検索する', () => {
    const results = getSuggestions('久保田', records, '');
    expect(results.some(r => r.brewery === '朝日酒造')).toBe(true);
  });
